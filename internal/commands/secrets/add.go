...
func NewAddCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Add or update one or more secrets for the current project.

Secrets are specified as KEY=VALUE pairs. If a secret with the same key already exists, it will be updated.

Examples:
  cerebrium secrets add API_KEY=sk-123456                    # Add a single secret
  cerebrium secrets add DB_HOST=localhost DB_PORT=5432       # Add multiple secrets
...
func runAdd(cmd *cobra.Command, args []string) error {
	...
	spinner := ui.NewSimpleSpinner("Fetching existing secrets...")
	spinner.Start()
	...
	spinner = ui.NewSimpleSpinner("Updating secrets...")
	spinner.Start()
	...
	if len(keys) == 1 {
		fmt.Println(ui.SuccessStyle.Render(fmt.Sprintf("Secret %q added successfully.", keys[0])))
	} else {
		fmt.Println(ui.SuccessStyle.Render(fmt.Sprintf("%d secrets added successfully: %s", len(keys), strings.Join(keys, ", "))))
	}
